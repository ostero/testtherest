...
class Config:
    ...
    def __init__(
        ...
        src: str = None,
        dest: str = None,
        port: int = 8080,  # new field with default
    ):
    ...
    def load_from_yaml(cls, filename: str = "api_key.yaml") -> "Config":
        ...
        src = http.get("src")
        dest = http.get("dest")
        port = http.get("port", 8080)  # new field, default 8080
        return cls(openai_model=openai_model, apikey=apikey, llmkey=llmkey, src=src, dest=dest, port=port)
